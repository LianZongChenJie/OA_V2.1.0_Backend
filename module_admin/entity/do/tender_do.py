...
from sqlalchemy import BigInteger, Column, DateTime, Integer, String, DECIMAL, Date, Text
...
class OaProjectTender(Base):
    ...
    project_cycle_num = Column(Integer, nullable=True, comment='项目周期(X)月')
    ...
    is_tender_submitted = Column(String(2), nullable=True, comment='是否投标（是/否）')
    ...
    has_tender_invoice = Column(String(2), nullable=True, comment='标书款发票（是/否）')
    is_deposit_paid = Column(String(2), nullable=True, comment='是否缴纳保证金（是/否）')
    ...
    is_deposit_refunded = Column(String(2), nullable=True, comment='是否退回保证（是/否）')
