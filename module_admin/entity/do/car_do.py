...
class OaCarRepair(Base):
    ...
    cost = Column(Numeric(15, 2), nullable=False, server_default='0.00', comment='费用')
    content = Column(Text, nullable=True, comment='维修/保养原因')
    file_ids = Column(String(255), nullable=False, server_default="''", comment='附件 ids')
    handled = Column(Integer, nullable=False, server_default='0', comment='处理人 ID')
...
class OaCarFee(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True, comment='ID')
    car_id = Column(Integer, nullable=False, server_default='0', comment='车辆 ID')
    title = Column(String(100), nullable=False, server_default="''", comment='费用名称')
    fee_time = Column(BigInteger, nullable=False, server_default='0', comment='费用时间时间戳')
    types = Column(SmallInteger, nullable=False, server_default='1', comment='费用类型')
