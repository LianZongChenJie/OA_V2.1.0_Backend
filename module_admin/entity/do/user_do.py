...
class SysUser(Base):
    ...
    is_staff = Column(Integer, nullable=True, comment='身份类型:1企业员工,2劳务派遣,3兼职员工）')
    ...
    work_location = Column(Integer, nullable=True, comment='办公地点')
    ...
    resident_type = Column(Integer, nullable=True, comment='户籍类型：1城镇，2农村')
    ...
    political = Column(Integer, nullable=True, server_default="''", comment='政治面貌')
    marital_status = Column(Integer, nullable=True, comment='婚姻状况：1已婚，2未婚,3离异')
    ...
    sex = Column(String(1), nullable=True, server_default='0', comment='用户性别（0男 1女 2未知）')
    ...
    login_num = Column(Integer, nullable=True, comment='登录次数')
    login_date = Column(DateTime, nullable=True, comment='最后登录时间')
    is_lock = Column(String(1), nullable=True, server_default='0', comment='是否锁屏')
    ...
    auth_did = Column(Integer, nullable=True, comment='数据权限类型:0仅自己关联的数据,1所属主部门的数据,2所属次部门的数据,3所属主次部门的数据,4所属主部门及其子部门数据,5所属次部门及其子部门数据,6所属主次部门及其子部门数据,7所属主部门所在顶级部门及其子部门数据,8所属次部门所在顶级部门及其子部门数据,9所属主次部门所在顶级部门及其子部门数据,10所有部门数据')
