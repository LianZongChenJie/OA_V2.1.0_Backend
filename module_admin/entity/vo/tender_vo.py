...
class CrudResponseModel(BaseModel):
    ...
    data: Optional[T] = None
    is_success: Optional[bool] = Field(True, description='是否成功')  # 补充缺失字段
...
class TenderModel(BaseModel):
    ...
    purchase_date: Optional[str] = Field(None, description='购买日期')  # 补充数据库字段
    tender_agency: Optional[str] = Field(None, description='招标机构', max_length=100)  # 补充数据库字段
    project_cycle: Optional[str] = Field(None, description='项目周期', max_length=50)  # 补充数据库字段
    shortlisted_countries: Optional[str] = Field(None, description='入围家数', max_length=20)  # 补充数据库字段
    budget_amount: Optional[float] = Field(None, description='预算金额（元）')  # 补充数据库字段
    bid_opening_date: Optional[str] = Field(None, description='开标日期')  # 补充数据库字段
    is_tender_submitted: Optional[str] = Field(None, description='是否投标', max_length=20)  # 修正描述
    non_tender_reason: Optional[str] = Field(None, description='未投原因', max_length=200)  # 补充数据库字段
    tender_document_fee: Optional[float] = Field(None, description='标书款（元）')  # 补充数据库字段
    has_tender_invoice: Optional[str] = Field(None, description='标书款发票', max_length=20)  # 补充数据库字段
    is_deposit_paid: Optional[str] = Field(None, description='是否缴纳保证金', max_length=20)  # 补充数据库字段
    tender_deposit: Optional[float] = Field(None, description='投标保证金（元）')  # 核心修改：替换 deposit_amount
    deposit_account_name: Optional[str] = Field(None, description='保证金账户名', max_length=100)  # 补充数据库字段
    deposit_bank: Optional[str] = Field(None, description='保证金开户行', max_length=100)  # 补充数据库字段
    deposit_account_no: Optional[str] = Field(None, description='保证金账号', max_length=50)  # 补充数据库字段
    is_deposit_refunded: Optional[str] = Field(None, description='是否退回保证金', max_length=20)
    bid_result: Optional[str] = Field(None, description='中标结果', max_length=20)
    bid_service_fee: Optional[float] = Field(None, description='中标服务费')  # 补充数据库字段
...
class DeleteTenderModel(BaseModel):
    ...
    删除投标信息模型
    """
    ids: str = Field(..., description='投标ID列表')
...
class TenderAttachmentModel(BaseModel):
    ...
    file_size: Optional[int] = Field(None, description='文件大小（字节）')  # 补充数据库字段
    file_ext: Optional[str] = Field(None, description='文件扩展名', max_length=20)  # 补充数据库字段
    file_mime: Optional[str] = Field(None, description='文件MIME类型', max_length=100)  # 补充数据库字段
    sort: Optional[int] = Field(None, description='排序')
...
class DeleteTenderAttachmentModel(BaseModel):
    ...
    删除投标附件模型
    """
    ids: str = Field(..., description='附件ID列表')
...
class TenderAttachmentListModel(DataResponseModel):
    ...
    投标附件列表返回模型
    """
    data: list[TenderAttachmentModel] = []