...
class OaPersonalQuitBaseModel(BaseModel):
    ...
    uid: int | None = Field(..., description='用户ID')
    lead_admin_id: int | None  = Field(..., description='上级领导')
    connect_id: int | None  = Field(..., description='资料交接人')
    connect_time: int | None  = Field(..., description='资料交接时间')
    connect_time_str:  str | None  = Field(None, description='资料交接时间（字符串格式）')
    connect_uids: str = Field(..., description='参与交接人')
    ...
    quit_time: int | None  = Field(..., description='离职时间')
    quit_time_str:  str | None  = Field(None, description='离职时间（字符串格式）')
    ...
    did: int | None  = Field(..., description='创建人所在部门')
    check_status: int | None  = Field(..., description='审核状态')
    check_status_name:  str | None  = Field(None, description='审核状态名称')
    check_flow_id: int | None  = Field(..., description='审核流程id')
    check_step_sort: int | None  = Field(..., description='当前审批步骤')
    check_uids: str | None  = Field(..., description='当前审批人ID')
    check_uids_list: Optional[List[int]] = Field(None, description='当前审批人ID列表')
    check_last_uid: str | None  = Field(..., description='上一审批人')
    check_history_uids: str | None  = Field(..., description='历史审批人ID')
    check_history_uids_list: Optional[List[int]] = Field(None, description='历史审批人ID列表')
    check_copy_uids: str | None  = Field(..., description='抄送人ID')
    check_copy_uids_list: Optional[List[int]] = Field(None, description='抄送人ID列表')
    check_time: int | None  = Field(..., description='审核通过时间')
    check_time_str:  str | None  = Field(None, description='审核通过时间（字符串格式）')
    create_time: int | None  = Field(..., description='创建时间')
    create_time_str:  str | None  = Field(None, description='创建时间（字符串格式）')
    update_time: int | None  = Field(..., description='更新时间')
...
class OaPersonnelQuitDetailModel(BaseModel):
    ...
    info : OaPersonalQuitBaseModel = Field(..., description='离职申请详情')
    records : list[OaFlowRecordBaseModel] = Field(..., description='离职申请审批记录')
