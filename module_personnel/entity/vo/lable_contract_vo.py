...
class OaLaborContractBaseModel(BaseModel):
    ...
    change_pid: int | None= Field(..., description='变更母合同')
    uid: int | None= Field(..., description='员工ID')
    cate: int | None= Field(..., description='合同类别')
    cate_name: str | None = Field(None, description='合同类别名称')
    types: int | None= Field(..., description='合同类型')
    types_name: str | None = Field(None, description='合同类型名称')
    enterprise_id: int | None= Field(..., description='关联企业主体ID')
    enterprise_name: str | None = Field(None, description='企业名称')
    properties: int | None= Field(..., description='合同属性')
    properties_name: str | None = Field(None, description='合同属性名称')
    code: str = Field(..., description='合同编号')
    title: str = Field(..., description='合同名称')
    sign_time: int | None= Field(..., description='签订时间')
    start_time: int | None= Field(..., description='生效时间')
    end_time: int | None= Field(..., description='失效时间')
    secure_time: int | None= Field(..., description='解除时间')
    trial_months: int | None= Field(..., description='试用月数')
    trial_end_time: int | None= Field(..., description='试用结束时间')
    trial_salary: Decimal = Field(..., description='试用工资')
    worker_salary: Decimal = Field(..., description='转正工资')
    status: int | None= Field(..., description='合同状态')
    ...
    admin_id: int | None= Field(..., description='创建人ID')
