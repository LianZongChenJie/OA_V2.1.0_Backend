...
class OaCareBaseModel(BaseModel):
    ...
    care_cate: int | None= Field(default=None, description='关怀项目')
    care_cate_name: str | None = Field(None, description='关怀项目名称')
    care_time: int | str | None= Field(default=None, description='关怀日期')
    vacation: int | None= Field(default=None, description='休假天数')
    ...
    @field_serializer('care_time')
    def serialize_care_time(self, value: Optional[int]) -> Optional[str]:
        """序列化关怀时间"""
