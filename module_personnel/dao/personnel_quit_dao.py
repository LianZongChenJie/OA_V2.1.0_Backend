...
class PersonnelQuitDao:
    ...
    async def get_page_list(cls, db: AsyncSession, query_object: OaPersonnelQuitPageQueryModel,
        ...
        if query_object.admin_id:
            query = (select(OaPersonalQuit)
                     .where(
                        OaPersonalQuit.check_status == query_object.check_status if query_object.check_status else True,
                        OaPersonalQuit.check_time.between(
                        ...
                        OaPersonalQuit.admin_id == query_object.admin_id,
                data_scope_sql,
            ...
            query = (select(OaPersonalQuit)
                    ...
                    func.find_in_set(query_object.check_uids, OaPersonalQuit.check_uids) > 0,
                    data_scope_sql,
            ).order_by(desc(OaPersonalQuit.create_time)))
            ...
            query = (select(OaPersonalQuit)
                     .where(
                        and_(
                            ...
                            or_(
                                query_object.admin_id == OaPersonalQuit.admin_id,
                                func.find_in_set(query_object.check_uids, OaPersonalQuit.check_uids) > 0,
                                func.find_in_set(query_object.check_copy_uids, OaPersonalQuit.check_copy_uids) > 0,
                                func.find_in_set(query_object.check_history_uids, OaPersonalQuit.check_history_uids) > 0
                            ),
    ...
    @classmethod
    async def cancel_change(cls, db: AsyncSession, query_model: OaPersonalQuitBaseModel):
        result = await db.execute(update(OaPersonalQuit).values(
            update_time=int(datetime.now().timestamp()),
            check_status=query_model.check_status,
            remark=query_model.remark,
        ).where(OaPersonalQuit.id == query_model.id))
        await db.commit()
        return result.rowcount
