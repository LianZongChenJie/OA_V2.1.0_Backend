...
class OutsModel(BaseModel):
    ...
    @Xss(field_name='reason', message='外出原因不能包含脚本字符')
    @NotBlank(field_name='reason', message='外出原因不能为空')
...
class OutsPageQueryModel(OutsModel):
    """
    外出分页查询模型
    """
...
class EditOutsModel(AddOutsModel):
    """
    编辑外出模型
    """
...
class DeleteOutsModel(BaseModel):
    """
    删除外出模型
    """
    ...
    id: int = Field(description='需要删除的外出 ID')