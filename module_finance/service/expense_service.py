...
class OaExpenseService:
    ...
    async def get_info_service(cls, query_db: \
        ...
        try:
            ...
            records = await FlowRecordDao.get_records_dict(query_db, info['OaExpense'].id, info['OaExpense'].check_flow_id)
            ...
            info.pop('OaExpense')
            detail.update(info)
            detail['records'] = ModelConverter.convert_to_camel_case(records)
            inter_list = []
            for item in inter:
