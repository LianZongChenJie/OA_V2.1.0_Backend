...
class MailService:
    ...
    # 网易企业邮箱配置
    SMTP_SERVER = 'smtp.yunyou.top'
    SMTP_PORT = 465
    SENDER_EMAIL = 'zhaopin1@bjlianzong.com'
    # 注意：此处需要使用邮箱的授权码，而非登录密码
    # 授权码获取方式：登录企业邮箱 -> 设置 -> 客户端授权密码
    SENDER_PASSWORD = '7@s3fw2UxGV9#'
    SENDER_NAME = 'OA系统'
    ...
    async def send_mail_service(cls, to_email: str, subject: str, content: str, is_html: bool = False) -> CrudResponseModel:
        ...
        try:
            ...
            message['From'] = f'{Header(cls.SENDER_NAME, "utf-8").encode()} <{cls.SENDER_EMAIL}>'
            message['To'] = to_email
            ...
            with smtplib.SMTP_SSL(cls.SMTP_SERVER, cls.SMTP_PORT) as server:
                ...
                server.sendmail(cls.SENDER_EMAIL, to_email, message.as_string())
