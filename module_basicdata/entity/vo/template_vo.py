...
class TemplateBaseModel(BaseModel):
    """基础模板模型"""
    title: str = Field(..., max_length=255, description='消息模板名称')
    name: str = Field(..., max_length=255, description='权限标识唯一，字母')
    types: int = Field(1, ge=1, le=2, description='类型:1普通消息,2审批消息')
    check_types: int = Field(0, description='审批类型:0')
    remark: str = Field('', max_length=500, description='备注描述，使用场景等')
    msg_link: str = Field('', max_length=255, description='消息模板链接(审批申请)')
    msg_title_0: str = Field('', max_length=255, description='消息模板标题(审批申请发审批人)')
    msg_content_0: str = Field('', max_length=500, description='消息模板内容(审批申请发审批人)')
    msg_title_1: str = Field('', max_length=255, description='消息模板标题(审批通过发申请人)')
    msg_content_1: str = Field('', max_length=500, description='消息模板内容(审批通过发申请人)')
    msg_title_2: str = Field('', max_length=255, description='消息模板标题(审批拒绝发申请人)')
    msg_content_2: str = Field('', max_length=500, description='消息模板内容(审批拒绝发申请人)')
    msg_title_3: str = Field('', max_length=255, description='消息模板标题(审批通过发抄送人)')
    msg_content_3: str = Field('', max_length=500, description='消息模板内容(审批通过发抄送人)')
    email_link: str = Field('', max_length=255, description='邮箱消息模板链接')
    status: int = Field(1, ge=-1, le=1, description='状态：-1删除 0禁用 1启用')
    admin_id: int = Field(0, description='创建人')
