...
class CustomerSourceService:
    @classmethod
    async def get_page_list_service(cls, query_db: AsyncSession, query_object: OaCustomerSourcePageQueryModel,
                                    data_scope_sql: ColumnElement, is_page: bool = False) -> PageModel[
                                                                                                 OaCustomerSourceBaseModel] | \
                                                                                             list[dict[str, Any]]:
        query_list = await CustomerSourceDao.get_page_list(query_db, query_object, data_scope_sql, is_page)
        if is_page:
            result_list = PageModel[OaCustomerSourceBaseModel](**{
    ...
    async def add_service(cls, query_db: AsyncSession, model: OaCustomerSourceBaseModel) -> CrudResponseModel:
        ...
        try:
            ...
            model.status = 1
            await CustomerSourceDao.add(query_db, model)
    ...
    @classmethod
    async def change_status_service(cls, query_db: AsyncSession, model: OaCustomerSourceBaseModel):
        try:
            await CustomerSourceDao.change_status(query_db, model)
    ...
    async def get_info_service(cls, query_db: \
            AsyncSession, id: int) -> OaCustomerSourceBaseModel:
        try:
            industry_info = await CustomerSourceDao.get_info_by_id(query_db, id)
    ...
    @classmethod
    async def check_name_unique_services(cls, query_db: AsyncSession, page_object: OaCustomerSourceBaseModel) -> bool:
        """
        校验用户名是否唯一service
