...
from module_basicdata.dao.custom.basic_customer_dao import BasicCustomerDao
...
class BasicCustomerService:
    @classmethod
    async def get_page_list_service(cls, query_db: AsyncSession, query_object: OaBasicCustomerPageQueryModel,
                                    data_scope_sql: ColumnElement, is_page: bool = False) -> PageModel[
                                                                                                 OaBasicCustomerBaseModel] | \
                                                                                             list[dict[str, Any]]:
        query_list = await BasicCustomerDao.get_page_list(query_db, query_object, data_scope_sql, is_page)
        if is_page:
            result_list = PageModel[OaBasicCustomerBaseModel](**{
    ...
    async def add_service(cls, query_db: AsyncSession, model: OaBasicCustomerBaseModel) -> CrudResponseModel:
        ...
        try:
            ...
            await BasicCustomerDao.add(query_db, model)
    ...
    async def get_info_service(cls, query_db: \
            AsyncSession, id: int) -> OaBasicCustomerBaseModel:
        try:
            industry_info = await BasicCustomerDao.get_info_by_id(query_db, id)
    ...
    async def check_name_unique_services(cls, query_db: AsyncSession, page_object: OaBasicCustomerBaseModel) -> bool:
        ...
        model = await BasicCustomerDao.get_info_by_title(query_db, OaBasicCustomerBaseModel(title=page_object.title,
